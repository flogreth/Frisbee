# FRISBEE TOASTER 3.01

# to make an executable file type:
# python -m PyInstaller --onefile --noconsole --add-data "media/finished.mp3;media" frisbee_toaster_3.01.py
        ...
        if item["model"] == selected_board:
            for v in item.get("downloads", []):
                if v["version"] == selected_version:
                    ...
                    log(f"Github API Key: {GITHUB_TOKEN}")
                    log("")
...
toast_checkbox = tk.Checkbutton(root, text="Auto-Toast", variable=toast_var)
